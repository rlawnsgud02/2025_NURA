...
bool SDFatLogger::write_data() {
    ...
    // for (int i = 0; i < 3; i++) { dataFile.print(DATA::acc[i]); dataFile.print(","); } Serial.println("8");
    // for (int i = 0; i < 3; i++) { dataFile.print(DATA::gyro[i]); dataFile.print(","); } Serial.println("9");
    // for (int i = 0; i < 3; i++) { dataFile.print(DATA::mag[i]); dataFile.print(","); } Serial.println("10");
    // for (int i = 0; i < 3; i++) { dataFile.print(DATA::euler[i]); dataFile.print(","); } Serial.println("11");
    // dataFile.print(DATA::maxG); dataFile.print(","); Serial.println("12");

    // for (int i = 0; i < 3; i++) { dataFile.print(DATA::baro[i]); dataFile.print(","); } Serial.println("13");

    

    // dataFile.print(DATA::gps.fixType); dataFile.print(",");
    // dataFile.print(DATA::gps.lon, 7); dataFile.print(",");
    // dataFile.print(DATA::gps.lat, 7); dataFile.print(",");
    // dataFile.print(DATA::gps.height); dataFile.print(",");
    // dataFile.print(DATA::gps.velN); dataFile.print(",");
    // dataFile.print(DATA::gps.velE); dataFile.print(",");
    // dataFile.print(DATA::gps.velD); dataFile.print(",");
    // Serial.println("14");

    // dataFile.println(DATA::eject);
    // Serial.println("15");
...
//     if (!dataFile) {
//         Serial.println("파일 열기 실패 (write_one_line)");
//         return false;
//     }

//     const auto& d = DATA::current;

//     char buf[256];
//     int len = snprintf(buf, sizeof(buf),
//         "%ld,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,"
//         "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.2f,"
//         "%.2f,%.2f,%.2f,%d,%.7f,%.7f,%.2f,%.2f,%.2f,%.2f,%d\n",
//         d.timestamp,
...
//         d.maxG,
//         d.baro[0], d.baro[1], d.baro[2],
//         d.fixType, d.lon, d.lat, d.height,
//         d.velN, d.velE, d.velD,
//         d.eject);

//     if (len <= 0 || len >= sizeof(buf)) {
//         Serial.println("버퍼 초과 또는 포맷 실패");
//         dataFile.close();
//         return false;
//     }
