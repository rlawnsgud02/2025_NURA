...
int SDFatLogger::create_new_data_file() {
    ...
    // dataFile.println("time,ax,ay,az,gx,gy,gz,mx,my,mz,roll,pitch,yaw,maxG,T,P,P_alt,fix,lon,lat,alt,vn,ve,vd,eject"); // GPS 포함 버전
    dataFile.println("time,ax,ay,az,gx,gy,gz,mx,my,mz,roll,pitch,yaw,maxG,T,P,P_alt,eject");
    dataFile.flush();
    // dataFile.close();
